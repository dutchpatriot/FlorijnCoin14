Florijncoin Core version 0.13.2 is now available from:

  <https://download.florijncoin.org/florijncoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/florijncoin-project/florijncoin/issues>
...
updates will be released anymore. Without security updates, using a florijncoin
...
ECDSA signatures inside Florijncoin transactions now use validation using
...
Previous versions of Florijncoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Florijncoin Core was to raise the default minimum
relay fee.

Florijncoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Florijncoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running florijncoind without having to do any manual configuration.
...
Florijncoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Florijncoin Core will relay transactions with
...
entirely in Florijncoin Core 0.13, and it is at this time undecided whether the
...
Florijncoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Florijncoin Core automatically creates a hidden service to listen on, without
manual configuration. Florijncoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Florijncoin Core is listening, and
...
Florijncoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Florijncoin
Core to use floating fees. By default, Florijncoin Core will use floating
fees.
...
(default: `0.0002` FLC/kB).

At all times, Florijncoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FLC.
Furthermore, Florijncoin Core will never create transactions paying less than
...
`florijncoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 255339 to a RPC server bound on localhost on port 44449 do:

    stunnel -d 255339 -r 127.0.0.1:44449 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /florijncoinrpc>
        ProxyPass http://127.0.0.1:55339/
        ProxyPassReverse http://127.0.0.1:55339/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass florijncoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< florijncoinrpc:<password>
    </Location>
...
Restarting florijncoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in florijncoin.conf
...
florijncoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/florijncoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Florijncoin Core code base has
...
- `florijncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `florijncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Florijncoin scripting system that in combination with
...
Florijncoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Florijncoin Core.
...
The two phases are distinct in the Florijncoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Florijncoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Florijncoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Florijncoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of florijncoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of florijncoins being spent is exactly
...
- **Increased security for multisig:** Florijncoin addresses (both P2PKH addresses
  ...
  believed to be provided by Florijncoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Florijncoin's current ledger (called the UTXO set),
  ...
  note that Florijncoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Florijncoin users to individually opt-in to almost any change in the Florijncoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Florijncoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Florijncoin
client to Florijncoin Core 0.13.2 to signal for segwit support. When a
...
Since Florijncoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `florijncoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `florijncoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
