...
*Setup instructions for a Gitian build of Florijncoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Florijncoin
...
- [Building Florijncoin Core](#building-florijncoin-core)
...
Clone the git repositories for florijncoin and Gitian.
...
in the florijncoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Florijncoin Core
----------------

To build Florijncoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the florijncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/florijncoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/florijncoin-project/florijncoin
...
For example:
```bash
URL=https://github.com/thrasher-/florijncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit florijncoin=${COMMIT} --url florijncoin=${URL} ../florijncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit florijncoin=${COMMIT} --url florijncoin=${URL} ../florijncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit florijncoin=${COMMIT} --url florijncoin=${URL} ../florijncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the florijncoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../florijncoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/florijncoin
SIGPATH=/some/root/path/florijncoin-detached-sigs

./bin/gbuild --url florijncoin=${BTCPATH},signature=${SIGPATH} ../florijncoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.flc` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/florijncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/florijncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/florijncoin-osx-build.assert
```
...
[gitian.sigs.flc](https://github.com/florijncoin-project/gitian.sigs.flc/) repository, or if that's not possible create a pull
