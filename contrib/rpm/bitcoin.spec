...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 55339
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 55449
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 44449
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 55559
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 55339
	%{_sbindir}/semanage port -d -p tcp 55449
	%{_sbindir}/semanage port -d -p tcp 44449
	%{_sbindir}/semanage port -d -p tcp 55559
	for selinuxvariant in %{selinux_variants}; do
